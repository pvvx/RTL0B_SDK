...
void _promisc_deinit(void *padapter);
int _promisc_recv_func(void *padapter, void *rframe);
int _promisc_set(rtw_rcr_level_t enabled, void (*callback)(unsigned char*, unsigned int, void*), unsigned char len_used);
unsigned char _is_promisc_enabled(void);
int _promisc_get_fixed_channel(void *fixed_bssid, u8 *ssid, int *ssid_length);
#endif

...
//	uint32_t cur_tx_page = 0;
	...
	for (i=0 ; i<I2SPageNum; i++) { // page 0, 1, 2, 3
		cur_rx_page = I2S_GetRxPage(I2S_DEV); // i2s_cur_rx_page
	...
	uint32_t page_num, uint32_t page_size)
{
	...
	I2S_SetDMABuf(I2S_DEV, (uint8_t*)tx_buf, (uint8_t*)rx_buf);

	for (i=0;i<page_num;i++) {
		I2S_SetTxPageAddr(i, tx_buf + ((page_size) * i));
		I2S_SetRxPageAddr(i, rx_buf + ((page_size) * i));
	}
}
...
void i2s_send_page(i2s_t *obj, uint32_t *pbuf)
{
	...
	if (I2S_GetTxPageAddr(cur_tx_page) != pbuf)

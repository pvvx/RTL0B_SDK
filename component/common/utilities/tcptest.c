...
//Restart:
	...
	end_time = start_time = xTaskGetTickCount();
	report_start_time = start_time;
	...
	printf("\n\r%s: [END] Totally receive %d KBytes in %d ms, %d Kbits/sec",
			__func__,
			(uint32_t) (total_size/KB),
			(uint32_t) (end_time - start_time),
			(end_time < start_time) ? ((uint32_t) (total_size * 8)/(end_time - start_time)) : 0);
...
extern int wext_set_tos_value(const char *ifname, u8 *tos_value);

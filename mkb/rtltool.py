...
#				print 'Not NAK or CAN: %02x' % (ord(char))
	...
	def ReadBlockFlash(self, stream, offset = 0, size = 0x200000):
		...
		if count > 0 and count < 0x10000 and offset >= 0: # 1 byte .. 16 Mbytes
			ret = self.sync()
			if ret:
				ret = self._port.write(struct.pack('<BHBH', ord(CMD_RBF), offset & 0xffff, (offset / 0x10000) & 0xff, count))
				count *= 4
				if ret:
	...
	def EraseSectorsFlash(self, offset = 0, size = 0x200000):
		...
		if count > 0 and count < 0x10000 and offset >= 0: # 1 byte .. 16 Mbytes
			for i in xrange(count):
				...
				if ret:
#					print '\r%d' % i
					ret = self.writecmd(struct.pack('<BHBH', ord(CMD_EFS), offset & 0xffff, (offset / 0x10000) & 0xff, 1))
					if not ret:
						return ret
...
if __name__ == '__main__':
	parser = argparse.ArgumentParser(description='RT871xBx ROM Bootloader Utility', prog='rtlptool')
...
#			sys.exit(0)
			...
			stream = io.BytesIO('\x05\x54\x00\x00')
			...
			stream = io.BytesIO('\x19\x20\x00\x10\x19\x20\x00\x10\x19\x20\x00\x10\x19\x20\x00\x10\x19\x20\x00\x10\x00\x00\x00\x00\x08\xb5\x02\x4c\x4f\xf4\x7a\x70\xa0\x47\xfb\xe7\x05\x22\x00\x00')
